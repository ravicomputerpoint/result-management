...
@section('content_header_title')
    Update Half Yearly Mark Entry
@endsection

@section('content_body')
    <form action="{{route('halfs.update',$half->id)}}" method="post">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-12">
                ...
                <select name="grade_id" id="grade_id" class="form-control" disabled>
                    ...
                    @foreach ($grades as $grade)
                        <option value="{{$grade->id}}" {{$half->grade_id == $grade->id ? 'selected' : ''}}>{{$grade->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-12">
                ...
                <select name="student_id" id="student_id" class="form-control" disabled>
                    ...
                    @foreach ($students as $student)
                        <option value="{{$student->id}}" {{$half->student_id == $student->id ? 'selected' : ''}}>{{$student->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <table class="table table-bordered">
            ...
            <tfoot>
                <tr>
                    <td>
                        <a href="{{route('halfs.index')}}" class="btn btn-outline-danger">Back</a>
                    </td>
...
@push('js')
    <script>
        ...
        let grade_id = {{$half->grade_id}}
        let hindi = {{$half->hindi ?? 0}}
        let english = {{$half->english ?? 0}}
        let math = {{$half->math ?? 0}}
        let drawing = {{$half->drawing ?? 0}}
        let rhymes = {{$half->rhymes ?? 0}}
        let gk = {{$half->gk ?? 0}}
        let sst = {{$half->sst ?? 0}}
        let science = {{$half->science ?? 0}}
        let computer = {{$half->computer ?? 0}}
