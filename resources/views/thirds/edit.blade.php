...
@section('content_header_title')
    Update Third Monthly Mark Entry
@endsection

@section('content_body')
    <form action="{{route('thirds.update',$third->id)}}" method="post">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-12">
                ...
                <select name="grade_id" id="grade_id" class="form-control" disabled>
                    ...
                    @foreach ($grades as $grade)
                        <option value="{{$grade->id}}" {{$third->grade_id == $grade->id ? 'selected' : ''}}>{{$grade->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-12">
                ...
                <select name="student_id" id="student_id" class="form-control" disabled>
                    ...
                    @foreach ($students as $student)
                        <option value="{{$student->id}}" {{$third->student_id == $student->id ? 'selected' : ''}}>{{$student->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <table class="table table-bordered">
            ...
            <tfoot>
                <tr>
                    <td>
                        <a href="{{route('thirds.index')}}" class="btn btn-outline-danger">Back</a>
                    </td>
...
@push('js')
    <script>
        ...
        let grade_id = {{$third->grade_id}}
        let hindi = {{$third->hindi ?? 0}}
        let english = {{$third->english ?? 0}}
        let math = {{$third->math ?? 0}}
        let drawing = {{$third->drawing ?? 0}}
        let rhymes = {{$third->rhymes ?? 0}}
        let gk = {{$third->gk ?? 0}}
        let sst = {{$third->sst ?? 0}}
        let science = {{$third->science ?? 0}}
        let computer = {{$third->computer ?? 0}}
